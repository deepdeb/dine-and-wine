            ...
            <ScrollView contentContainerStyle={{ height: '100%' }} >
                <View className="m-2 flex justify-center items-center">
                    ...
                    <View className="w-5/6">
                        <Formik initialValues={{ email: "", password: "" }} validationSchema={validationSchema} onSubmit={handleSignup}>
                            {({ handleChange, handleBlur, handleSubmit, values, errors, touched }) => (
                                <View className="w-full">
                                    ...
                                    <TextInput className="h-11 border border-white text-white rounded px-2" keyboardType='email-address' onChangeText={handleChange("email")} value={values.email} onBlur={handleBlur("email")} />

                                    {touched.email && errors.email && (
                                        <Text className="text-red-500 text-xs mb-2" >{errors.email}
                                        </Text>
                                    )}

                                    <Text className="text-[#f49b33] mb-2 mt-4">Password</Text>
                                    <TextInput className="h-11 border border-white text-white rounded px-2" secureTextEntry onChangeText={handleChange("password")} value={values.password} onBlur={handleBlur("password")} />

                                    {touched.password && errors.password && (
                                        <Text className="text-red-500 text-xs mb-2" >{errors.password}
                                        </Text>
                                    )}
                        ...
                        <View>
                            <TouchableOpacity className="flex flex-row justify-center items-center gap-2 mt-5 p-2" onPress={() => router.push('signin')}>
                            ...
                            <TouchableOpacity className="flex flex-row justify-center items-center gap-2 mb-5 p-2" onPress={() => router.push('home')}>
